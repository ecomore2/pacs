...
```{r}
(tmp <- pacs %>% filter(province == "Vientiane [prefecture]") %>% 
  mutate(onset2 = if_else(is.na(onset),
                          if_else(is.na(hospitalization),
                                  if_else(is.na(consultation),
                                          sample_collection + corrections["onset_sample_collection"],
         ...
         test_info = pcr == "positive" | ns1 == "positive" | (pcr == "negative" & ns1 == "negtive"),
         test_info = ifelse(is.na(test_info), FALSE, test_info),
...
```{r}
pacs %>% filter(province == "Vientiane [prefecture]") %>% 
  mutate(onset2 = if_else(is.na(onset),
                          if_else(is.na(hospitalization),
                                  if_else(is.na(consultation),
                                          sample_collection + corrections["onset_sample_collection"],
         ...
         test_info = ifelse(is.na(test_info), FALSE, test_info),
         time_info = ! is.na(onset2)) %>%
  group_by(village_info, time_info) %>% 
  tally() %>% 
  ungroup()
```
